...
export const imgurOAuthCallback: Middleware = async (req, res) => {
    ...
    if (accessToken) {
        response.redirect(
            `https://localhost:5500/client/pages/platforms-imgur.html#access_token=${accessToken}`
        );
    } else {
        response.redirect(
            `https://localhost:5500/client/pages/platforms-imgur.html?error=Authorization%20failed`
        );
    }
};
